"""Celery task — download from provider CDN and upload image to R2 (Issue #63)."""
...
@celery_app.task(bind=True, max_retries=3, default_retry_delay=60, name="tasks.upload_image_to_storage")  # type: ignore[untyped-decorator]
def upload_image_to_storage(self: Any, asset_id: str, media_url: str) -> None:
    """Download the generated image from the provider CDN and upload to R2.
    ...
    try:
        ...
        if not client_id:
            log.warning("upload_image_to_storage.asset_not_found", extra={"asset_id": asset_id})
            return

        data = storage.download_media(media_url)
        public_url = storage.upload_image(data, asset_id, client_id)
        ...
        log.info("upload_image_to_storage.done", extra={"asset_id": asset_id})
        _notify_completed(asset_id, public_url)

    except Exception as exc:
        log.error(
            "upload_image_to_storage.failed",
